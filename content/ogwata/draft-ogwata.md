...
上記のうち、2のリストはユーザーガイド〈文書の作成と保存 > フォントに関する補足情報 > クラウドにインストールされているフォント一覧[^11]〉にまとめているので参考にしてほしい。

では、上記のような3種類のフォントをVivliostyle Pubはどのように扱ってプレビューしたり、PDF出力したりしているのだろう？　まずプレビューにおけるフォントを指定するしくみを図示する（図1）
