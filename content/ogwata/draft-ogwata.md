...
プレビューのために実際に組版をおこなうのがVivliostyle.jsだ。図2をみると、これはユーザーのPC上のフロントエンドにある。したがってVivliostyle.jsと同じユーザーのPCにある**フォント1**、及びWebフォントサービスの**フォント3**がプレビューで利用できるわけだ。他方、クラウドにインストールされた**フォント2**はこの図には見当たらない。したがってプレビューで**フォント2**も利用できないということだ。
...
PDF出力のために組版をおこなうVivliostyle.jsはクラウド上のバックエンドにある。だからVivliostyle.jsと同じクラウドにある**フォント2**、及びWebフォントサービスの**フォント3**がPDF出力で利用できるわけだ。

ここまでの説明をまとめてみよう。プレビューとPDF出力は、それぞれ別の組版エンジン、Vivliostyle.jsが担当する。その際、それぞれの組版エンジンは以下の3つの場所にあるフォントを使用する。

- ユーザーのPCにあるローカルフォント（図2：**フォント1**）
- クラウドにインストールされたフォント（図3：**フォント2**）
- Webフォントサービスのフォント（図1、図3：**フォント3**）

プレビューを担当するユーザーのPC上のVivliostyle.jsは、ローカルフォント（図2：**フォント1**）、及びWebフォントサービスのフォント（図2、図3：**フォント3**）を使う。

一方、PDF出力を担当するクラウド上のVivliostyle.jsは、クラウドにインストールされたフォント（図3：**フォント2**）、及びWebフォントサービスのフォント（図2、図3：**フォント3**）を使う。

このような仕組みなので、プレビューで使われるフォントとPDF出力で使われるフォントが、いつでも無条件に一致するのは、Webフォントサービスのフォント（図2、図3：**フォント3**）だけなのである。プレビューではクラウドにインストールされたフォント（図3：**フォント2**）は使えず、PDF出力ではユーザーのPCにインストールされたローカルフォント（図2：**フォント1**）は使えない。

例えば、プレビューで使ったローカルフォント（図2：**フォント1**）がクラウドになかった場合、PDF出力ではクラウドにインストールされたフォントのうち、似たものに置き換えられる（→ クラウド上のVivliostyle CLIにおける代替フォントルール）。その結果、プレビューとPDF出力のフォントが不一致になり、ページのズレが発生することにご注意いただきたい。
